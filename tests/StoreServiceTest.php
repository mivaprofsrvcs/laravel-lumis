...
it('builds graphics, json, and root URLs', function () {
	...
	expect($store->rootUrl())->toBe('https://example.test/mm5/');
	expect($store->jsonUrl())->toBe('https://example.test/mm5/json.mvc');
	expect($store->graphicsUrl())->toBe('https://example.test/mm5/graphics/00000001/');
});
...
it('includes basic auth header when configured', function () {
	config()->set('miva.store.auth.username', 'u');
	config()->set('miva.store.auth.password', 'p');
